...
#include "Geometry/MTDGeometry/interface/CSCLayerGeometry.h"
#include "Geometry/MTDGeometry/interface/CSCLayer.h"
...
CSCLayerGeometryInside::CSCLayerGeometryInside( const edm::ParameterSet& ps )
 : dashedLineWidth_(194), dashedLine_( std::string(dashedLineWidth_, '-') ), 
   myName_( "CSCLayerGeometryInside" ), ntries( ps.getUntrackedParameter<int>("ntries") ),
   nbx( ps.getUntrackedParameter<int>("nbx") ), nby( ps.getUntrackedParameter<int>("nby") ),
...
CSCLayerGeometryInside::~CSCLayerGeometryInside()
{
  delete tro;
}
...
void CSCLayerGeometryInside::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
   ...
   // Build the geometry
   edm::ESHandle<ETLGeometry> pgeom;
   iSetup.get<MTDGeometryRecord>().get( pgeom );     
   ...
   for ( int i = 1; i <= ntries; ++i ) {
     ...
     if ( lg1a->inside( lp ) ) h1ai->Fill(x, y);
	  else h1ao->Fill(x, y);

     if ( lg1b->inside( lp ) ) h1bi->Fill(x, y);
	  else h1bo->Fill(x, y);
