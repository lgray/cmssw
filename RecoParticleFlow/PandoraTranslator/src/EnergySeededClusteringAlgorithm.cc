...
#include "RecoParticleFlow/PandoraTranslator/interface/EnergySeededClusteringAlgorithm.h"

#include <list>

using namespace pandora;

using lc_content_fast::KDTreeCube;
using lc_content_fast::KDTreeTesseract;
using lc_content_fast::build_3d_kd_search_region;
using lc_content_fast::build_4d_kd_search_region;


namespace cms_content
{

EnergySeededClusteringAlgorithm::EnergySeededClusteringAlgorithm() :
...
StatusCode EnergySeededClusteringAlgorithm::Run()
{
...
StatusCode EnergySeededClusteringAlgorithm::InitializeKDTrees(const TrackList *const pTrackList, const CaloHitList *const pCaloHitList)
{
...
StatusCode EnergySeededClusteringAlgorithm::SeedClustersWithTracks(const TrackList *const pTrackList, ClusterVector &clusterVector)
{
    if (0 == m_clusterSeedStrategy)
        return STATUS_CODE_SUCCESS;
...
StatusCode EnergySeededClusteringAlgorithm::GetCurrentClusterFitResults(const ClusterVector &clusterVector, ClusterFitResultMap &clusterFitResultMap) const
{
...
// inprogress
StatusCode EnergySeededClusteringAlgorithm::FindHitsInPreviousLayers(unsigned int pseudoLayer, CustomSortedCaloHitList *const pCustomSortedCaloHitList,
...
StatusCode EnergySeededClusteringAlgorithm::FindHitsInSameLayer(unsigned int pseudoLayer, CustomSortedCaloHitList *const pCustomSortedCaloHitList,
    const ClusterFitResultMap &clusterFitResultMap, ClusterVector &clusterVector)
{
    const float maxTrackSeedSeparation = std::sqrt(m_maxTrackSeedSeparation2);
...
StatusCode EnergySeededClusteringAlgorithm::GetGenericDistanceToHit(const Cluster *const pCluster, const CaloHit *const pCaloHit, const unsigned int searchLayer,
...
StatusCode EnergySeededClusteringAlgorithm::GetDistanceToHitInSameLayer(const CaloHit *const pCaloHit, const CaloHitList *const pCaloHitList,
...
StatusCode EnergySeededClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit *const pCaloHit, const CaloHitList *const pCaloHitList,
...
StatusCode EnergySeededClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit *const pCaloHit, const CartesianVector &clusterPosition,
    const CartesianVector &clusterDirection, float &distance) const
{
    const CartesianVector &hitPosition(pCaloHit->GetPositionVector());
...
StatusCode EnergySeededClusteringAlgorithm::GetDistanceToTrackSeed(const Cluster *const pCluster, const CaloHit *const pCaloHit, unsigned int searchLayer,
...
StatusCode EnergySeededClusteringAlgorithm::GetDistanceToTrackSeed(const Cluster *const pCluster, const CaloHit *const pCaloHit, float &distance) const
{
    const CartesianVector &hitPosition(pCaloHit->GetPositionVector());
...
StatusCode EnergySeededClusteringAlgorithm::RemoveEmptyClusters(const ClusterVector &clusterVector) const
{
...
StatusCode EnergySeededClusteringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
