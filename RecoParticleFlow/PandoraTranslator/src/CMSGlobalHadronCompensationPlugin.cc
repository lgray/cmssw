...
StatusCode GlobalHadronCompensation::MakeEnergyCorrections(const Cluster *const pCluster, float &correctedHadronicEnergy) const
{
  ...
  float en_FH(getByPiOverECorrectedEn(rawen_FH,rawen,int(ForwardSubdetector::HGCHEF))); 
  ...
  if(nHits_FH>0 && en_FH/en>0.1)
      ...
      for( ; layer != hits_end; ++ layer ) 
	{
      ...
      float c_FH((nHits_FH-nHits_elim_FH)/(nHits_FH-nHits_avg_FH));
      gcScale=(en_EE+c_FH*en_FH+en_BH)/(en);
    }
  ...
  /*
    ...
    << " Final: " << gcScale*residualScale*en
    << std::endl
    << "\t EE: " << rawen_EE << "->" << en_EE
    << " FH: " << rawen_FH << "->" << en_FH
    << " BH: " << rawen_BH << "->" << en_BH
  ...
  */
  ...
  //all done here
  correctedHadronicEnergy=gcScale*residualScale*en;
