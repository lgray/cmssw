...
public:
    ...
    CaloParticle( const SimTrack& simtrk, const edm::Ref<std::vector<SimVertex> >& parentVertex );
    ...
    void setParentVertex(const edm::Ref<std::vector<SimVertex> >& ref) { parentVertex_ = ref; }
    void clearParentVertex() { parentVertex_ = edm::Ref<std::vector<SimVertex> >(); }
    ...
    const edm::Ref<std::vector<SimVertex> >& parentVertex() const { return parentVertex_; }
    ...
    Point vertex() const {
      const math::XYZTLorentzVector &p = (*parentVertex_).position();
...
private:
    ...
    // Source vertex (if hard scatter)
    edm::Ref<std::vector<SimVertex> > parentVertex_;
